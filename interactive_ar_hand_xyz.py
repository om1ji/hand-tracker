...
class InteractiveARHandTracker:
    ...
    def get_hand_orientation(self, landmarks, frame_width, frame_height):
        """Вычисляет ориентацию руки используя пальцы как оси координат (как в Blender)"""
        # Ключевые точки для определения ориентации
        wrist = landmarks[0]
        thumb_tip = landmarks[4]    # кончик большого пальца - ось X
        thumb_mcp = landmarks[2]    # основание большого пальца
        index_tip = landmarks[8]    # кончик указательного пальца - ось Z (вверх)
        index_mcp = landmarks[5]    # основание указательного пальца
        pinky_tip = landmarks[20]   # кончик мизинца - ось Y (в сторону)
        pinky_mcp = landmarks[17]   # основание мизинца
        
        # Преобразуем в пиксельные координаты
        wrist_pos = [wrist.x * frame_width, wrist.y * frame_height, wrist.z]
        thumb_tip_pos = [thumb_tip.x * frame_width, thumb_tip.y * frame_height, thumb_tip.z]
        thumb_mcp_pos = [thumb_mcp.x * frame_width, thumb_mcp.y * frame_height, thumb_mcp.z]
        ...
        pinky_tip_pos = [pinky_tip.x * frame_width, pinky_tip.y * frame_height, pinky_tip.z]
        pinky_mcp_pos = [pinky_mcp.x * frame_width, pinky_mcp.y * frame_height, pinky_mcp.z]
        
        # Векторы осей координат на основе пальцев
        # Z-axis: указательный палец (вверх в Blender)
        z_axis_vector = [
            index_tip_pos[0] - index_mcp_pos[0],
            index_tip_pos[1] - index_mcp_pos[1],
            index_tip_pos[2] - index_mcp_pos[2]
        ]
        
        # Y-axis: мизинец (в сторону в Blender)
        y_axis_vector = [
            pinky_tip_pos[0] - pinky_mcp_pos[0],
            pinky_tip_pos[1] - pinky_mcp_pos[1],
            pinky_tip_pos[2] - pinky_mcp_pos[2]
        ]
        
        # X-axis: большой палец (перпендикулярно к среднему)
        x_axis_vector = [
            thumb_tip_pos[0] - thumb_mcp_pos[0],
            thumb_tip_pos[1] - thumb_mcp_pos[1],
            thumb_tip_pos[2] - thumb_mcp_pos[2]
        ]
        
        # Вычисляем углы поворота для каждой оси
        
        # ROLL (поворот вокруг Z-axis) - угол указательного пальца в плоскости XY
        roll = math.atan2(z_axis_vector[1], z_axis_vector[0])
        
        # PITCH (поворот вокруг X-axis) - угол большого пальца относительно горизонтали
        pitch = math.atan2(x_axis_vector[1], math.sqrt(x_axis_vector[0]**2 + x_axis_vector[2]**2))
        
        # YAW (поворот вокруг Y-axis) - угол среднего пальца в плоскости XZ
        yaw = math.atan2(y_axis_vector[2], y_axis_vector[0])
        
        # Центр ладони для позиционирования
        palm_center = [
            (thumb_tip_pos[0] + index_tip_pos[0] + pinky_tip_pos[0] + wrist_pos[0]) / 4,
            (thumb_tip_pos[1] + index_tip_pos[1] + pinky_tip_pos[1] + wrist_pos[1]) / 4,
            (thumb_tip_pos[2] + index_tip_pos[2] + pinky_tip_pos[2] + wrist_pos[2]) / 4
        ]
        
        # Дополнительные метрики для отладки
        z_axis_length = math.sqrt(z_axis_vector[0]**2 + z_axis_vector[1]**2 + z_axis_vector[2]**2)
        y_axis_length = math.sqrt(y_axis_vector[0]**2 + y_axis_vector[1]**2 + y_axis_vector[2]**2)
        x_axis_length = math.sqrt(x_axis_vector[0]**2 + x_axis_vector[1]**2 + x_axis_vector[2]**2)
        
        return {
            'roll': math.degrees(roll),      # Z-axis: указательный палец
            'pitch': math.degrees(pitch),    # X-axis: большой палец
            'yaw': math.degrees(yaw),        # Y-axis: мизинец
            'z_axis_length': z_axis_length,  # длина указательного пальца
            'y_axis_length': y_axis_length,  # длина мизинца
            'x_axis_length': x_axis_length,  # длина большого пальца
            'center': palm_center[:2],
            'thumb_tip_pos': thumb_tip_pos[:2],
            'index_tip_pos': index_tip_pos[:2],
            'pinky_tip_pos': pinky_tip_pos[:2],
            'thumb_mcp_pos': thumb_mcp_pos[:2],
            'index_mcp_pos': index_mcp_pos[:2],
            'pinky_mcp_pos': pinky_mcp_pos[:2],
            'raw_vectors': {
                'z_axis': z_axis_vector,  # указательный палец
                'y_axis': y_axis_vector,  # мизинец
                'x_axis': x_axis_vector   # большой палец
            }
        }
    ...
    def draw_rotation_guides(self, frame, landmarks, frame_width, frame_height):
        """Рисует визуальные ориентиры для вращения (пальцы как оси координат)"""
        if self.control_mode == "rotate" and (self.current_gesture == "rotate" and self.gesture_timer >= self.gesture_activation_delay):
            ...
            thumb_tip_pos = [int(thumb_tip.x * frame_width), int(thumb_tip.y * frame_height)]
            thumb_mcp_pos = [int(thumb_mcp.x * frame_width), int(thumb_mcp.y * frame_height)]
            index_tip_pos = [int(index_tip.x * frame_width), int(index_tip.y * frame_height)]
            index_mcp_pos = [int(index_mcp.x * frame_width), int(index_mcp.y * frame_height)]
            pinky_tip_pos = [int(pinky_tip.x * frame_width), int(pinky_tip.y * frame_height)]
            pinky_mcp_pos = [int(pinky_mcp.x * frame_width), int(pinky_mcp.y * frame_height)]
            
            # Z-axis: Указательный палец (ROLL) - красный
            cv2.line(frame, tuple(index_mcp_pos), tuple(index_tip_pos), (0, 0, 255), 4)
            cv2.circle(frame, tuple(index_tip_pos), 8, (0, 0, 255), -1)
            cv2.circle(frame, tuple(index_tip_pos), 12, (255, 255, 255), 2)
            cv2.putText(frame, "Z-ROLL", (index_tip_pos[0] + 15, index_tip_pos[1] - 15), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            
            # Y-axis: Мизинец (YAW) - зеленый
            cv2.line(frame, tuple(pinky_mcp_pos), tuple(pinky_tip_pos), (0, 255, 0), 4)
            cv2.circle(frame, tuple(pinky_tip_pos), 8, (0, 255, 0), -1)
            cv2.circle(frame, tuple(pinky_tip_pos), 12, (255, 255, 255), 2)
            cv2.putText(frame, "Y-YAW", (pinky_tip_pos[0] + 15, pinky_tip_pos[1] - 15), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            
            # X-axis: Большой палец (PITCH) - синий
            cv2.line(frame, tuple(thumb_mcp_pos), tuple(thumb_tip_pos), (255, 0, 0), 4)
            cv2.circle(frame, tuple(thumb_tip_pos), 8, (255, 0, 0), -1)
            cv2.circle(frame, tuple(thumb_tip_pos), 12, (255, 255, 255), 2)
            cv2.putText(frame, "X-PITCH", (thumb_tip_pos[0] + 15, thumb_tip_pos[1] - 15), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
            
            # Рисуем основания пальцев
            cv2.circle(frame, tuple(index_mcp_pos), 4, (0, 0, 255), -1)
            cv2.circle(frame, tuple(pinky_mcp_pos), 4, (0, 255, 0), -1)
            cv2.circle(frame, tuple(thumb_mcp_pos), 4, (255, 0, 0), -1)
    ...
    def draw_gesture_info(self, frame, gesture, control_mode):
        ...
        # Отладочная информация для осей координат (только в режиме вращения)
        if self.control_mode == 'rotate' and hasattr(self, 'last_hand_position') and self.last_hand_position:
            z_axis_length = self.last_hand_position.get('z_axis_length', 0)
            y_axis_length = self.last_hand_position.get('y_axis_length', 0)
            x_axis_length = self.last_hand_position.get('x_axis_length', 0)
            
            cv2.putText(frame, f"Z-axis (Index): {int(z_axis_length)}", (20, 170), font, 0.35, (0, 0, 255), 1)
            cv2.putText(frame, f"Y-axis (Pinky): {int(y_axis_length)}", (20, 185), font, 0.35, (0, 255, 0), 1)
            cv2.putText(frame, f"X-axis (Thumb): {int(x_axis_length)}", (20, 200), font, 0.35, (255, 0, 0), 1)
        ...
        cv2.putText(frame, "Rotation: Index=Z-ROLL, Pinky=Y-YAW, Thumb=X-PITCH", (20, 240), font, 0.4, (100, 255, 255), 1)
    ...
    def run(self):
        ...
        print("- Открытая ладонь (5 пальцев) - 3D вращение с осями координат как в Blender:")
        print("  🔴 Указательный палец = Z-axis ROLL (поворот вокруг Z)")
        print("  🟢 Мизинец = Y-axis YAW (поворот вокруг Y)")
        print("  🔵 Большой палец = X-axis PITCH (поворот вокруг X)")
